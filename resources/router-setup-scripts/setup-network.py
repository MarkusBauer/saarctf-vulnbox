...
subnet {get_ip(team_id, 0)} netmask 255.255.255.0 {{
    range {get_ip(team_id, 6)} {get_ip(team_id, 63)};
    option subnet-mask 255.255.255.128;
    option broadcast-address {get_ip(team_id, 127)};
    option routers {get_ip(team_id, 1)};
    option domain-name-servers 8.8.8.8;
    # routes for VPN (10.32.X.64/26) and default (0.0.0.0/0).
    option rfc3442-classless-static-routes 26, {get_ip(team_id, 64).replace('.', ', ')}, {get_ip(team_id, 1).replace('.', ', ')}, 0, {get_ip(team_id, 1).replace('.', ', ')};
    option ms-classless-static-routes 26, {get_ip(team_id, 64).replace('.', ', ')}, {get_ip(team_id, 1).replace('.', ', ')}, 0, {get_ip(team_id, 1).replace('.', ', ')};
}}
...
Address = {get_ip(team_id, 64 + i)}/32
...
'''
    ...
    print('[.] Find your client configuration at "/root/team-wireguard-client-XXX.conf". ')
    print('    IMPORTANT: Replace "SERVER_PUBLIC_IP" with your IP, then distribute to your teammates.')
...
def main() -> None:
    ...
    while True:
        ...
        if query_yes_no('Do you want to use DHCP for easier configuration?'):
            ...
            print(f'[*]  DHCP active.')
        if query_yes_no('Do you want a VPN server (OpenVPN or Wireguard) for your teammates to connect to?'):
            opt = query_options('Which VPN type?', ['(none)', 'OpenVPN', 'Wireguard'])
            if opt == 'OpenVPN':
                print(f'[.]  Configuring OpenVPN server ...')
                configure_vpnserver(team_id)
                print(f'[*]  OpenVPN active.')
            elif opt == 'Wireguard':
                print(f'[.]  Configuring Wireguard VPN server ...')
                configure_wireguard_vpnserver(team_id)
                print(f'[*]  Wireguard VPN active.')
        print('')
